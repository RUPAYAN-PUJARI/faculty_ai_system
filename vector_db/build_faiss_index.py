"""Build a FAISS index from text embeddings."""
...
DEFAULT_INDEX = Path(__file__).resolve().parents[1] / "vector_db" / "faiss_text.index"


def build_index(embeddings_path: Path, index_path: Path) -> faiss.Index:
...
def main() -> None:
	parser = argparse.ArgumentParser(description="Build a FAISS index for text embeddings")
